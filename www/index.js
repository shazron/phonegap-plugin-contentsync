...
var ContentSync = function(options) {
    ...
    if (typeof options.loadNativePlugins === 'undefined') {
        options.loadNativePlugins = false;
    }
    ...
    setTimeout(function() {
        exec(success, fail, 'Sync', 'sync', [options.src, options.id, options.type, options.headers, options.copyCordovaAssets, options.copyRootApp, options.timeout, options.trustHost, options.manifest, options.loadNativePlugins]);
    }, 10);
};
